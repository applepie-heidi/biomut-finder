...
def count_letters(ref_letter='T',
    ...
    if max_letter[0] == ref_letter:
        ...
        if pr >= 0.7:
            print(
                f'mutation: {max_letter[0]} instead {ref_letter}, {c}, {tuple_letters}')
            return 1
        else:
    ...
    for i, genr in enumerate(reads):  # loop for all reads
        ...
        if res:
            for ind, letter in enumerate(res):
                if letter != '':
                    res_seq[ind] = res_seq[ind] + (
                    letter,)  # aling our reed to reference sequence (actually add in tuple new letter)
                # print(letter)
        else:
